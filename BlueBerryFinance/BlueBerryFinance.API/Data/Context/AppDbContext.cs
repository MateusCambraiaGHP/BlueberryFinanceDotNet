    ...
    public class AppDbContext : DbContext
    {
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<User>(e =>
            {
                ...
                e.Property(x => x.Profile).HasMaxLength(50).IsRequired();
            ...
            modelBuilder.Entity<Store>(e =>
            {
                e.HasKey(x => x.Id);
                e.Property(x => x.Name).HasMaxLength(256).IsRequired();
                e.HasOne(x => x.Category).WithMany(c => c.Stores).HasForeignKey(x => x.CategoryId).OnDelete(DeleteBehavior.Restrict);
                e.HasQueryFilter(x => !x.IsDeleted);
            });
            ...
            modelBuilder.Entity<Transaction>(e =>
            {
                ...
                e.HasOne(x => x.Currency).WithMany().HasForeignKey(x => x.CurrencyId).OnDelete(DeleteBehavior.Restrict);
                e.HasOne(x => x.FixedExpense).WithMany().HasForeignKey(x => x.FixedExpenseId).OnDelete(DeleteBehavior.SetNull).IsRequired(false);
                e.HasQueryFilter(x => !x.IsDeleted);
            });
            ...
            {
                e.HasKey(x => x.Id);
                e.Property(x => x.ImageUrl).HasMaxLength(1024).IsRequired();
                e.Property(x => x.RawText).HasMaxLength(8192);
